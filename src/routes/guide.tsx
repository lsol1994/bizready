    ...
    <div class="flex h-screen overflow-hidden">
      ...
      <aside class="w-64 gradient-bg flex flex-col flex-shrink-0">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          ...
          {/* 카테고리 바로가기 */}
          <div class="pt-3 pb-1">
            <div class="text-sky-400 text-xs font-medium px-3 mb-2">카테고리</div>
            {[
              { name: '세무회계', icon: 'fa-calculator' },
              { name: '인사노무', icon: 'fa-users' },
              { name: '총무', icon: 'fa-building' },
            ].map(cat => (
              <a href={`/dashboard/archive?cat=${encodeURIComponent(cat.name)}`}
                 class="sidebar-item flex items-center gap-3 px-3 py-2 rounded-lg text-sky-200 hover:text-white text-sm">
                <i class={`fas ${cat.icon} w-4 text-center text-xs`}></i>
